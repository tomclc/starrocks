...
public:
    ...
    // Factory for remote FS (Iceberg sidecar files) — no TabletIndex dependency
    static Status create(int32_t cell_level, const std::string& index_file_path,
                         std::unique_ptr<S2IndexWriter>* writer);

    S2IndexWriter(std::shared_ptr<TabletIndex> tablet_index, std::string index_file_path);
    S2IndexWriter(int32_t cell_level, std::string index_file_path);
...
private:
    ...
    // Cell IDs are stored as int64_t to match Parquet INT64 storage and statistics ordering.
    std::vector<std::pair<int64_t, int64_t>> _cell_row_pairs;
    int64_t _current_row_offset = 0;
};
